...
class JointEmbedModel(nn.Module):
    def __init__(self, config, vlen, embeds=None):
        ...
        self.vggnet.classifier = self.vggnet.classifier[:-2]
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Dropout(p=0.5),
            nn.Linear(1000, int(config['n_classes'])))
    ...
    def forward(self, images, q_idxs):
        ...
        lstm_out, (hn, cn) = self.lstm_q(ques_embedded)
        hn = hn.permute(1, 0, 2)
        cn = cn.permute(1, 0, 2)
        hn = hn.view(images.shape[0], self.lstm_hidden_size*self.lstm_num_layers)
        cn = cn.view(images.shape[0], self.lstm_hidden_size*self.lstm_num_layers)
